    ...
    def load_colors(self):
        try:
            ...
            while True:
                ...
                if not colors_data:  # Nessun colore rimasto
                    break
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # Container per indicatore prodotto e nome
        product_container = QVBoxLayout()
        product_container.setSpacing(2)
        
        # Indicatore prodotto (1/N) con carattere più piccolo
        self.product_label = QLabel(f"Prodotto {self.current_product_index + 1}/{self.total_products}")
        self.product_label.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        self.product_label.setStyleSheet("color: white;")
        product_container.addWidget(self.product_label)
        
        # Nome del prodotto corrente (solo la prima parte prima del primo +)
        product_name = ""
        if self.products_list and self.current_product_index < len(self.products_list):
            full_name = self.products_list[self.current_product_index]
            product_name = full_name.split('+')[0] if '+' in full_name else full_name
        
        self.product_name_label = QLabel(product_name)
        self.product_name_label.setFont(QFont("Arial", 12))
        self.product_name_label.setStyleSheet("color: #CCCCCC;")
        self.product_name_label.setWordWrap(True)
        product_container.addWidget(self.product_name_label)
        
        self.product_info_layout.addLayout(product_container)
    ...
    def update_product_info(self):
        ...
        if self.current_product_index < self.total_products:
            ...
            # Estrai solo la prima parte del nome (prima del primo +)
            full_name = self.products_list[self.current_product_index]
            product_name = full_name.split('+')[0] if '+' in full_name else full_name
    ...
    def next_product_or_finish(self):
        ...
        # Sposta il prodotto appena completato nella cartella "2-Prodotti_caricati"
        if self.current_product_index < self.total_products:
            current_product = self.products_list[self.current_product_index]
            source_path = os.path.join(PRODUCTS_DIR, current_product)
            destination_dir = os.path.join("C:\\Users\\fanti\\Desktop\\Caricamento_Prodotti", "2-Prodotti_caricati")
            destination_path = os.path.join(destination_dir, current_product)
            ...
            try:
                ...
                # Sposta la cartella del prodotto
                import shutil
                shutil.move(source_path, destination_path)
                print(f"Prodotto '{current_product}' spostato in '2-Prodotti_caricati'")
                
            except Exception as e:
                print(f"Errore nello spostamento del prodotto '{current_product}': {e}")
                QMessageBox.warning(self, "Attenzione", f"Errore nello spostamento del prodotto '{current_product}': {e}")
        
        self.current_product_index += 1
        
        # Ricarica la lista dei prodotti (dopo lo spostamento)
        self.products_list = self.get_products_list()
        self.total_products = len(self.products_list)
    ...
    def upload_product_b2b(self, image_data, global_data):
        ...
        try:
            ...
            for sku, img_info in image_data.items():
                ...
                # Carica le immagini per il prodotto
