...
# CONFIGURAZIONE API PHOTOROOM
PHOTOROOM_API_KEY = "sk_pr_default_cad9e2c1235ea3d7969d1c4a413271055dd83be7"
...
def rimuovi_sfondo_photoroom(image_path):
    with open(image_path, 'rb') as file:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Errore API PhotoRoom: {response.status_code} - {response.text}")
...
def elabora_immagini():
    ...
    for i, filename in enumerate(files, start=1):
        ...
        try:
            image_no_bg = rimuovi_sfondo_photoroom(input_path)
        ...
        base_name = os.path.splitext(filename)[0] + '_PR.jpg'
        output_jpg = os.path.join(output_finali, base_name)
...
def mostra_risultati(risultati, tempo_totale):
    ...
    testo_risultati.insert(END, f"🏷️ Suffisso: _PR")
...
# GUI
root = Tk()
root.title("Scontornamento Immagini - PhotoRoom AI")
...
Label(header_frame, 
      text="🤖 PhotoRoom AI - Scontornamento", 
      font=("Segoe UI", 20, "bold"), 
      bg="#f8f9fa", 
      fg="#212529").pack()

Label(header_frame, 
      text="Sfrutta l'intelligenza artificiale per rimuovere gli sfondi", 
      font=("Segoe UI", 11), 
      bg="#f8f9fa", 
...
rounded_frame = Frame(button_frame, bg="#dc3545", padx=3, pady=3)
rounded_frame.pack()

avvia_btn = Button(rounded_frame,
                   text="🚀 Avvia PhotoRoom AI",
